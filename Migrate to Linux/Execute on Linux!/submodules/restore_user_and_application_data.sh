...
ensure_7z() {
  ...
  capture pm_install $pkg >/dev/null 2>&1 || true
  # Fedora/RHEL split the extra codecs into p7zip-plugins.
  { [ "$FAMILY" = "rhel" ] && ! have_cmd 7z; } && capture pm_install p7zip-plugins >/dev/null 2>&1
  # openSUSE's older package is p7zip; try it as a fallback.
  { [ "$FAMILY" = "suse" ] && ! have_cmd 7z && ! have_cmd 7zz; } && capture pm_install p7zip >/dev/null 2>&1
  for b in 7z 7zz 7za; do have_cmd "$b" && { printf '%s' "$b"; return 0; }; done
  return 1
}
...
restore_main() {
  ...
  info "for example '${TARGET_HOME}/Downloads/MigrateToLinux_UserData_20260101_120000.zip'."
  local arch ans
  while :; do
