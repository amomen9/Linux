...
.SYNOPSIS
    ...
    C_windows_configs.csv for later application on macOS.
...
.DESCRIPTION
    Extracts user-facing settings and writes them to C_windows_configs.csv:
     ...
     11. Firewall rules (all rules, for the macOS Application Firewall + pf)
    POST-INSTALL items (Phase=post; applied AFTER apps are installed):
     ...
     13. Startup items (-> login items / LaunchDaemons) + auto-start third-party services
     14. ~/.ssh (all files) + 15. Contacts folder

    The personal files for 14-15 are bundled into ONE encrypted archive
    "Execute on macOS!/migrated_user_data.tar.enc" (tar + OpenSSL AES-256-CBC/PBKDF2
    ...
    files for the post items are staged into "Execute on macOS!/migrated_user_data/".
...
function Add-Row {
    ...
    param([string]$Category, [string]$ConfigKey, [string]$WindowsValue, [string]$macOSCommand,
...
Add-Row 'Power' 'lid_close_on_ac'      $acAction '' "logind.conf: HandleLidSwitchExternalPower=$($winTomacOSLid[$acAction])" -Scope 'System'
Add-Row 'Power' 'lid_close_on_battery' $dcAction '' "logind.conf: HandleLidSwitch=$($winTomacOSLid[$dcAction])" -Scope 'System'
...
Add-Row 'Display' 'resolution' $resolution '' "displayplacer / System Settings > Displays"
Add-Row 'Display' 'scaling' "${scalePercent}%" '' "System Settings > Displays: pick a scaled resolution (macOS scales the whole UI)"
...
Add-Row 'Keyboard' 'layout' $keyboardLayout $inputMethodTips "System Settings > Keyboard > Input Sources (defaults write com.apple.HIToolbox)"

# Common shortcuts: these are Windows conventions we want to know about
# We note the standard Windows shortcuts that a power user wants on macOS
$windowsShortcuts = @(
...
Add-Row 'Keyboard' 'shortcuts_note' ($windowsShortcuts -join ' ; ') '' 'Ctrl maps to Command on macOS; review System Settings > Keyboard > Keyboard Shortcuts'
...
Add-Row 'Telemetry' 'telemetry_level' $telemetryLevel '' "defaults write /Library/Application Support/CrashReporter/... AutoSubmit -bool false"
Add-Row 'Telemetry' 'location_service' $locationEnabled '' "System Settings > Privacy & Security > Location Services (SIP-protected on recent macOS)"
...
#    locks the screen, the value is "never" - so macOS disables lock/blanking
...
Add-Row 'Screen' 'lock_screen_timeout' $lockValue '' 'sysadminctl -screenLock + com.apple.screensaver idleTime; "never" => sysadminctl -screenLock off'
...
try {
    ...
    # Sensitivity 1..20 (default 10)  ->  macOS com.apple.mouse.scaling (0..3, default 1).
...
Add-Row 'Mouse' 'mouse_size'  $mouseSize  '' 'com.apple.universalaccess mouseDriverCursorSize'
Add-Row 'Mouse' 'mouse_speed' $mouseSpeed '' 'NSGlobalDomain com.apple.mouse.scaling'
Add-Row 'Mouse' 'mouse_accel' $mouseAccel '' 'NSGlobalDomain com.apple.mouse.scaling = -1 disables acceleration'
Add-Row 'Mouse' 'mouse_swap'  $mouseSwap  '' 'NSGlobalDomain com.apple.mouse.swapLeftRightButton'
Add-Row 'Mouse' 'mouse_dblclick' $mouseDbl '' 'NSGlobalDomain com.apple.mouse.doubleClickThreshold (seconds)'
...
# Accessibility values are 'true'/'false' literals (the boolean form `defaults -bool` takes).
...
Add-Row 'Accessibility' 'a11y_stickykeys'   $a11yStick '' 'com.apple.universalaccess stickyKey'
Add-Row 'Accessibility' 'a11y_slowkeys'     $a11ySlow  '' 'com.apple.universalaccess slowKey'
Add-Row 'Accessibility' 'a11y_mousekeys'    $a11yMouse '' 'com.apple.universalaccess mouseDriver'
Add-Row 'Accessibility' 'a11y_highcontrast' $a11yHC    '' 'com.apple.universalaccess increaseContrast'
Add-Row 'Accessibility' 'a11y_magnifier'    $a11yMag   '' 'com.apple.universalaccess closeViewScrollWheelToggle (Zoom)'
...
try {
    ...
    # KeyboardSpeed 0..31 (chars/sec ~2.5..30)  ->  ms BETWEEN repeats (macOS KeyRepeat, 15 ms ticks).
...
Add-Row 'Keyboard' 'key_repeat_delay' $kbDelayMs '' 'NSGlobalDomain InitialKeyRepeat (converted from ms to 15 ms ticks)'
Add-Row 'Keyboard' 'key_repeat_rate'  $kbRateMs  '' 'NSGlobalDomain KeyRepeat (converted from ms to 15 ms ticks)'
Add-Row 'Keyboard' 'numlock'          $numlock   '' 'not applicable - Mac keyboards have no Num Lock key'
...
Add-Row 'Time' 'timezone'   $ianaTz    '' 'macOS: systemsetup -settimezone' -Scope 'System'
Add-Row 'Time' 'ntp_server' $ntpServer '' 'macOS: systemd-timesyncd NTP= (non-Microsoft)' -Scope 'System'
...
Add-Row 'Appearance' 'color_scheme' $colorScheme '' 'NSGlobalDomain AppleInterfaceStyle (Dark / absent = Light)'

# ACCENT COLOR -> nearest macOS accent (NSGlobalDomain AppleAccentColor).
...
Add-Row 'Appearance' 'accent_color' $accent '' 'NSGlobalDomain AppleAccentColor (0 red .. 6 pink, -1 graphite)'
...
Add-Row 'Locale' 'locale' $posixLocale '' 'macOS: defaults write NSGlobalDomain AppleLocale / AppleLanguages' -Scope 'System'
...
Add-Row 'Proxy' 'proxy_mode'       $proxyMode '' 'networksetup -setwebproxystate / -setautoproxystate'
Add-Row 'Proxy' 'proxy_host'       $proxyHost '' 'networksetup -setwebproxy <service> <host> <port>'
Add-Row 'Proxy' 'proxy_port'       $proxyPort '' 'networksetup -setwebproxy / -setsecurewebproxy port'
Add-Row 'Proxy' 'proxy_autoconfig' $proxyAuto '' 'networksetup -setautoproxyurl <service> <PAC url>'
...
Add-Row 'Touchpad' 'touchpad_tap'     $tpTap     '' 'com.apple.AppleMultitouchTrackpad Clicking'
Add-Row 'Touchpad' 'touchpad_natural' $tpNatural '' 'NSGlobalDomain com.apple.swipescrolldirection'
...
Add-Row 'Power' 'sleep_ac' $sleepAc '' 'pmset -c sleep <minutes>' -Scope 'System'
Add-Row 'Power' 'sleep_dc' $sleepDc '' 'pmset -b sleep <minutes>' -Scope 'System'
...
Add-Row 'Appearance' 'night_light' $nightLight '' 'Night Shift (System Settings > Displays; the nightlight CLI can toggle it)'
...
Add-Row 'DefaultApps' 'default_browser' $defBrowser '' 'macOS: defaultbrowser <token> (System Settings > Desktop & Dock)' -Phase 'post' -Scope 'User'
...
try {
    ...
    if (Test-Path $hp) {
        Get-Content $hp -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            Add-Row 'Hosts' 'host_entry' (($line -replace '\|', ' ')) '' 'macOS: merge into /etc/hosts (then dscacheutil -flushcache)' -Scope 'System'
            $hostsCount++
        }
    }
} catch {}
...
try {
    Get-Printer -ErrorAction SilentlyContinue | Where-Object {
        ...
        Add-Row 'Printers' 'printer' $packed '' 'macOS: CUPS lpadmin (best-effort)' -Scope 'System'
        $prnCount++
    }
} catch {}
...
try {
    Get-NetIPConfiguration -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        if ($man -or $dns) {
            ...
            Add-Row 'NetConfig' 'static_net' $packed '' 'macOS: networksetup -setmanual / -setdnsservers (manual note)' -Scope 'System'
            $netCount++
        }
    }
} catch {}
...
try {
    ...
    foreach ($n in $names) {
        ...
        Add-Row 'Wifi' 'wifi_profile' $packed '' 'macOS: networksetup -addpreferredwirelessnetworkatindex' -Scope 'System'
        $wifiCount++
    }
} catch {}
...
# -----------------------------------------------------------------------
# 12. FIREWALL rules (all rules; program rules -> the macOS Application Firewall, port rules -> pf)
# -----------------------------------------------------------------------
$fwCount = 0
try {
    ...
    foreach ($r in $rules) {
        ...
        # macOS port firewalls cannot express that, so the apply step recreates program-scoped
        ...
        Add-Row 'Firewall' 'fw_rule' $packed '' 'macOS: socketfilterfw (per-app) / pf anchor (per-port)' -Scope 'System'
...
# =======================================================================
#  POST-INSTALL items (Phase=post): applied AFTER apps are installed.
#  Personal files travel to macOS inside "Execute on macOS!/migrated_user_data/"
...
$stageRoot = Join-Path $projRoot 'Execute on macOS!\migrated_user_data'
...
#     Each resolves to an installed macOS app ON THE LINUX SIDE; here we only
...
foreach ($src in $shortcutSources) {
    ...
    try {
        Get-ChildItem -Path $src.Path -Filter '*.lnk' -File -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            Add-Row 'Shortcuts' $src.Kind $packed '' 'macOS: resolve to an installed .app (Dock item / Desktop alias)' -Phase 'post' -Scope 'User'
...
#      macOS; current-user (HKCU Run / user Startup) -> User. The macOS side only
        ...
        try {
            ...
            if ($k) {
                foreach ($p in $k.PSObject.Properties) {
                    ...
                    Add-Row 'Startup' 'startup_item' $packed '' 'macOS: login item (user) or LaunchDaemon (system) if it resolves to an installed app' -Phase 'post' -Scope $Scope
...
try {
    Get-CimInstance Win32_Service -ErrorAction SilentlyContinue | Where-Object {
        ...
        Add-Row 'Services' 'service' $packed '' 'macOS: launchctl enable if a clearly-matching launchd job exists' -Phase 'post' -Scope 'System'
...
#      System; otherwise User). The macOS side turns each into a launchd job IF its
...
try {
    Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object {
        ...
        Add-Row 'ScheduledTasks' 'task' $packed '' 'macOS: launchd job if the program resolves to an installed app' -Phase 'post' -Scope $scope
        $taskCount++
    }
} catch {}
...
if (Test-Path $sshSrc) {
    ...
    Add-Row 'SSH' 'ssh_dir' 'staged: migrated_user_data (encrypted archive)' '' 'macOS: decrypt the archive, copy into ~/.ssh; perms 600/700' -Phase 'post' -Scope 'User'
} else {
...
if (Test-Path $contactsSrc) {
    ...
    Add-Row 'Contacts' 'contacts_dir' 'staged: migrated_user_data (encrypted archive)' '' 'macOS: decrypt the archive, copy into ~/Contacts (.contact files are XML)' -Phase 'post' -Scope 'User'
} else {
...
# e.g. the bundled CJK families alone are hundreds of MB). On macOS the user is
...
Add-Row 'Wallpaper' 'wallpaper_file' $(if ($wpStaged) { 'staged: migrated_user_data (encrypted archive)' } else { '' }) '' 'macOS: osascript "set picture of every desktop"' -Phase 'post' -Scope 'User'
...
#     migrated_user_data.tar.enc   (decrypted on macOS by apply_settings.sh).
...
$archivePath = Join-Path $projRoot 'Execute on macOS!\migrated_user_data.tar.enc'
...
if ($stagedAnything) {
    ...
    if ($xferPwd -and $opensslExe -and $tarExe) {
        ...
        if (Test-Path $archivePath) {
            Write-Host "Personal data encrypted -> Execute on macOS!\migrated_user_data.tar.enc" -ForegroundColor Green
        } else {
