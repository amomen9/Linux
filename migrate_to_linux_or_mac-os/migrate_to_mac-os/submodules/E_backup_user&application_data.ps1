...
.SYNOPSIS
    ...
    macOS and restored by "Execute on macOS!/submodules/restore_user_and_application_data.sh".
...
.DESCRIPTION
    ...
    on the macOS side, reversibly), staged into a neat tree, then archived as
...
# Render a polished two-column table (Windows source -> macOS destination). Column
...
function Write-ReportTable {
    param([object[]] $Rows, [string] $LeftLabel = 'Windows source', [string] $RightLabel = 'macOS destination', [string] $Indent = '  ')
...
foreach ($inc in $plan.userProfile.inclusions) {
    ...
    if (Add-Tree -SrcRoot $src -ArchPrefix $prefix -Incl @() -Excl $excl) {
        $report.Add(@{ Source = $src; Target = $inc.macTarget })
    }
}
        ...
        foreach ($scopeName in @('userdir','system')) {
            ...
            foreach ($s in @($scope.sources)) {
                ...
                if (Add-Tree -SrcRoot $sp -ArchPrefix "apps/$slug/$scopeName" -Incl $incl -Excl $excl) {
                    $report.Add(@{ Source = $sp; Target = "[$appKey/$scopeName] $($scope.macTarget)" })
                }
            }
        }
    }
...
# ---------------------------------------------------------------------------
$logPath = Join-Path $OutputDir 'MigrateToMacOS_backup_report.log'
$reportSorted = $report | Sort-Object { $_.Source }
Write-Host ""
Write-Info "The backup will include these locations (verbatim) and where each is meant to land on macOS:"
Write-Host ""
...
$logLines = @('Migrate to macOS -- backup report (' + (Get-Date) + ')','')
...
Write-Info ("Estimated size needed on macOS : ~{0}" -f (Format-Size $totalBytes))
...
if ($ArchiveFormat -eq 'enctar') {
    ...
    $finalName = if ($encrypt) { "MigrateToMacOS_UserData_$stamp.tar.gz.enc" } else { "MigrateToMacOS_UserData_$stamp.tar.gz" }
    ...
    $finalName = if ($ArchiveFormat -eq 'zip') { "MigrateToMacOS_UserData_$stamp.zip" } else { "MigrateToMacOS_UserData_$stamp.7z" }
    $finalPath = Join-Path $OutputDir $finalName
    if ($useZipFallback) {
...
Write-OkLine ("  Est. size on macOS : ~{0} (uncompressed)" -f (Format-Size $totalBytes))
if ($encrypt) {
    Write-Info  "  Encrypted with your transfer password. Carry it to macOS and give its path to the installer when asked."
} else {
