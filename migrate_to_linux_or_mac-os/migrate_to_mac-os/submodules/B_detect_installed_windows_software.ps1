...
.SYNOPSIS
    ...
    used on macOS: availability flags, the best alternative, an estimated performance
    competency, the macOS pricing model, and whether it is worth installing on macOS.
...
.DESCRIPTION
    The script produces B_installed_windows_software.csv with 10 columns:
      ...
      JSON manifest that maps every Windows app to its macOS options):
        macOS Availability         one or more flags, normalised to a fixed order:
                                     Available on macOS ; Not Available ;
                                     Native Alternative ; Available as WebApp ;
                                     macOS Docker ; Windows Emulator (Wine/CrossOver)
        Best macOS Alternative     best current option when not natively available
        Alternative Competency     rough % of how the macOS app/alternative performs
                                   vs the Windows original (>100 = macOS is better);
                                   filled for natively-available apps too (~100%)
        Pricing model              Free (FOSS) / Free / Freemium / Shareware / Paid
                                   of the recommended macOS option
        Must be included on macOS  yes/no - DERIVED by the script: "yes" for the most
                                   competent installable option per product (native or
                                   alternative) whose competency >= -MustIncludeThreshold
        Can be synched to macOS alternative  whether the Windows app's data can be
                                     automatically synced with the macOS alternative
    ...
    via the Get-OfflineManifest function.  "Must be included on macOS" is computed
...
.PARAMETER MustIncludeThreshold
    Minimum Alternative Competency (%) for "Must be included on macOS" = yes. Default 70.
...
.PARAMETER Online
    For apps not in the knowledge base, query repology.org live to detect macOS
...
# (Migrate to macOS/documents/) so it sits beside the other detection CSVs and is read
...
function Get-OfflineManifest {
    ...
    .SYNOPSIS
        ...
        the manifest, then extracts macOS Availability, Best Alternative,
    ...
    if (-not $Global:ManifestLoaded) {
        ...
        try {
            ...
            Write-Host "Loaded offline manifest: $($data.applications.Count) Windows apps, $($data.stats.totalAlternatives) total macOS alternatives." -ForegroundColor Cyan
        }
        catch {
    ...
    $avail    = if ($entry.macAvailability) { $entry.macAvailability } else { 'Needs Review' }
...
#    web-only services, and apps you plan to use on macOS that are not yet
...
$enriched = foreach ($app in $deduped) {
    ...
    if (-not $kb) {
        ...
        [PSCustomObject]@{
            ...
            Source                        = $app.Source
            'macOS Availability'          = 'Needs Review - NOT IN MANIFEST'
            'Best macOS Alternative'      = 'Not in manifest - research manually or add to B_applications.json'
            'Alternative Competency'      = ''
            'Pricing model'               = ''
            'Must be included on macOS'   = 'no'
            'Can be synched to macOS alternative' = 'No, manual transfer'
            'macOS Alternative Type'      = ''
            'Download URL'                = ''
        }
    }
    else {
        ...
        [PSCustomObject]@{
            ...
            Publisher                     = [string]$app.Publisher
            Source                        = $app.Source
            'macOS Availability'          = $kb.Availability
            'Best macOS Alternative'      = $kb.BestAlternative
            'Alternative Competency'      = $kb.Competency
            'Pricing model'               = $kb.PricingModel
            'Must be included on macOS'   = $mustInclude
            'Can be synched to macOS alternative' = $kb.Syncability
            'macOS Alternative Type'      = $kb.AppType
...
if ($unmatched -gt 0) {
    ...
    Write-Host '  the CSV file to add proper macOS alternatives, or you can'     -ForegroundColor Yellow
    ...
    Write-Host '  manually if you need their macOS alternatives evaluated.'      -ForegroundColor Yellow
    Write-Host '==============================================================' -ForegroundColor Yellow
    Write-Host ''
...
#    macOS side can rebuild the same images/volumes/networks/containers. This
#    writes docker_rebuild.sh into submodules/ and "Execute on macOS!/". Its
...
if (Get-Command docker -ErrorAction SilentlyContinue) {
    ...
    if (Test-Path $dockerDisc) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "OK - docker_rebuild.sh generated (submodules + 'Execute on macOS!')." -ForegroundColor Green
        } else {
