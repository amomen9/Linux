<#
.SYNOPSIS
    Generates the universal "Execute on macOS!" installer scripts from the
...
.DESCRIPTION
    ...
    detect the running macOS release (Catalina 10.15 .. the latest) and the CPU
    architecture (Apple Silicon arm64 / Intel x86_64) at runtime, installing
    Cask-first (Homebrew Cask) with Homebrew-formula, Mac App Store and
    direct-download fallbacks:

      execute_all.sh                 - orchestrator
      install_device_drivers.sh      - OS driver updates / Rosetta / printing + device report
      install_must_have_software.sh  - the application install list
      apply_settings.sh              - Windows settings -> macOS

    All output is written with LF newlines and no BOM so it runs on macOS as-is.

.PARAMETER OutputDir
    Destination folder for the generated scripts.  Default: the "Execute on macOS!"
...
if (-not $OutputDir)     { $OutputDir     = Join-Path $projRoot 'Execute on macOS!' }
if (-not $ManifestPath)  { $ManifestPath  = Join-Path $documentsDir 'B_applications.json' }
if (-not $AdditionalCsv) { $AdditionalCsv = Join-Path $projRoot 'Additional_Manual_macOS_Software_Requirments.csv' }
...
function New-InstallCall {
    ...
    # caskId is the macOS counterpart of the Linux flatpakId: one token that is
    # identical on every macOS version and on both architectures.  masId is the
    # numeric Mac App Store identifier used for Store-only applications.
    Add-Flag $parts '--cask' (Get-Prop $Install 'caskId')
    Add-Flag $parts '--mas'  (Get-Prop $Install 'masId')
    # method=builtin: the macOS counterpart ships with the OS (Finder, Preview,
    # Calculator, Disk Utility, Time Machine, ...).  builtinApp names the app so the
    # report can tell the user what replaces their Windows app and how to open it.
    Add-Flag $parts '--builtin' (Get-Prop $Install 'builtinApp')

    $native = Get-Prop $Install 'native'
    if ($native) {
        # brew = Homebrew formula (the curated one); port = MacPorts (optional -- the
        # engine falls back to the brew name when it is absent).
        foreach ($fam in 'brew','port') {
    ...
    # Cross-version support: the minimum macOS major version this app needs.  The
    # engine skips the app with an explicit reason on an older Mac instead of failing.
    Add-Flag $parts '--minos' (Get-Prop $Install 'minOS')

    $dl = Get-Prop $Install 'downloadUrl'
    if ($dl) {
        Add-Flag $parts '--url-x86' (Get-Prop $dl 'x86_64')
        Add-Flag $parts '--url-arm' (Get-Prop $dl 'arm64')
        Add-Flag $parts '--url-dmg' (Get-Prop $dl 'dmg')
        Add-Flag $parts '--url-pkg' (Get-Prop $dl 'pkg')
    }
...
# Guess a Homebrew package name from an alternative's display name: take the first component
...
# order, until one works; otherwise it falls back to the manual download-page flow). A cask
# is intentionally NOT guessed (it needs reverse-DNS app ids, which a plain name never matches).
function New-FallbackInstall {
    param($Alt)
    $obj = [ordered]@{ method = 'cask' }
    $im = [string](Get-Prop $Alt 'installMethod')
    if ($im -match '^(?i)web$') {
        $obj['method'] = 'webapp'; $obj['webappUrl'] = [string](Get-Prop $Alt 'downloadUrl')
    } else {
        # A Homebrew CASK token and a FORMULA name are both plain lowercase-hyphenated
        # strings, and the same guess is a plausible candidate for either, so it is emitted
        # as both: the engine tries the cask first, then the formula, then falls back to the
        # manual download-page flow.  (On Linux this guessed only native package names,
        # because Flatpak needs a reverse-DNS id that a plain name never matches.)
        $guess = Get-PkgGuess ([string](Get-Prop $Alt 'name'))
        if ($guess) { $obj['caskId'] = $guess; $obj['native'] = [ordered]@{ brew = $guess } }
...
# install_app() runs this (if defined) before a cask/formula install, so the vendor's
# own Homebrew tap is added first and the latest upstream build is used.
...
if ($unmatchedNames.Count -gt 0) {
    Write-Host ""
    Write-Host ("WARNING: {0} detected Windows app(s) are NOT in the manifest -- no macOS equivalent will be installed for them:" -f $unmatchedNames.Count) -ForegroundColor Yellow
...
# Force-included manifest apps: emitted even when NOT detected on Windows (e.g. a macOS-only
...
foreach ($app in $selectedApps) {
    # Rank ALL alternatives best-first by competency (no mustInclude gate). Wine/CrossOver
    ...
    foreach ($alt in (Get-Prop $app 'alternatives')) {
        ...
        if ($at -match '(?i)wine|proton|crossover') { continue }
    ...
    if ($bestInstall -and (Get-Prop $bestInstall 'promptFile')) {
        ...
        # MANUAL_APPS entry: "Windows app|macOS alternative|note|download page URL|paid(1/0)"
    ...
    # Vendor tap (req5): emit a repo_setup_<slug>() once per app if the best
    # alternative declares install.repo. install_app runs it before cask/formula installs.
    if ($bestInstall) {
    ...
    # Win32 app with no native "Available on macOS" build), emit a wine_app call. It
    ...
    $avail = [string](Get-Prop $app 'macAvailability')
    $src   = [string](Get-Prop $app 'sourceType')
    if (($src -match '(?i)win32') -and ($avail -notmatch '(?i)available on (macos|mac os|mac)\b')) {
...
if (Test-Path $AdditionalCsv) {
    ...
    foreach ($row in (Import-Csv -Path $AdditionalCsv)) {
        ...
        $pkgs = [string]$row.'macOS Package(s)'
...
if (Test-Path $DriverCsv) {
    foreach ($row in (Import-Csv -Path $DriverCsv)) {
        ...
        $st  = [string]$row.'macOS Driver Status'
        $must = [string]$row.'Must install on macOS'
        if (($must -match '^(?i)\s*yes') -or ($st -notmatch 'In-OS')) {
            $driverLines.Add('  info "' + (ConvertTo-BashString $dev.Trim()) + ' [' + (ConvertTo-BashString $cls) + '] -> ' + (ConvertTo-BashString $st) + '"')
        } else { $covered++ }
    }
}
if ($driverLines.Count -eq 0) { $driverLines.Add('  info "All detected devices are covered by the drivers built into macOS."') }
else { $driverLines.Add('  info "(' + $covered + ' additional devices are covered by the drivers built into macOS.)"') }
...
#     script so it can classify the backup archive at runtime on macOS).
# ---------------------------------------------------------------------------
$dmSrc = Join-Path $documentsDir 'D_data_migration.json'
if (Test-Path $dmSrc) {
    $dmDst = Join-Path $subDir 'D_data_migration.json'
    # Copy with LF + no BOM so jq on macOS reads it cleanly.
...
$cfgHeader = @(
    '# Migrate to macOS -- saved answers for execute_all.sh'
