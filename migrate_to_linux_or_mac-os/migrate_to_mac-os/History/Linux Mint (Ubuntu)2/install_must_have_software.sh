...
#   * A general multi-method resolver (install_app) installs any program by trying the
#     package managers the OS supports in order of significance (APT -> Snap -> Flatpak)
#     and then other methods: GitHub-release .deb resolved LIVE from the API (no hard-
#     coded version) -> vendor .deb -> manual prompt -> web-app shortcut. First success
#     wins. It is ADDITIVE — every curated step and feature below is preserved.
#
...
# =============================================================================
# MULTI-METHOD PACKAGE RESOLVER  (the "try every method, in order" installer)
# -----------------------------------------------------------------------------
# install_app() installs ONE program by trying, in order of significance:
#   PACKAGE MANAGERS the target OS supports
#     1. APT      — the distro's native repos / configured vendor APT repos
#     2. Snap     — snapd (bootstrapped on demand if missing)
#     3. Flatpak  — Flathub
#   then the OTHER METHODS implemented here:
#     4. GitHub release .deb — newest version resolved LIVE from the GitHub API
#                              (no hard-coded version, no HTML page scraping)
#     5. Direct vendor .deb URL
#     6. Manual download prompt (waits for the file, then installs it)
#     7. Web-app .desktop shortcut (last resort — app only exists on the web)
# The FIRST method that succeeds wins; remaining methods are skipped. Every
# outcome flows through the existing mark_ok / mark_manual / mark_fail
# reporting, and the winning method is recorded in the results log + summary.
#
# This is ADDITIVE: the curated per-app steps below still work exactly as
# before. install_app is the general path used where a clean multi-method
# fallback also removes a hard-coded version (e.g. XDM, PeaZip).
# =============================================================================

# Package managers available on this host, most-significant first (for reporting).
PKG_MGRS=""
detect_pkg_mgrs() {
  local m=()
  have_cmd apt-get && m+=("apt")
  have_cmd snap    && m+=("snap")
  have_cmd flatpak && m+=("flatpak")
  PKG_MGRS="${m[*]:-}"
}

# Pick the first asset download URL matching a regex out of a GitHub API JSON blob
# (handles both a single release object and an array of releases).
_gh_pick_asset() {  # _gh_pick_asset JSON REGEX
  if have_cmd jq; then
    printf '%s' "$1" | jq -r '.. | .browser_download_url? // empty' 2>/dev/null | grep -iE "$2" | head -n1
  else
    printf '%s' "$1" \
      | grep -oE '"browser_download_url"[[:space:]]*:[[:space:]]*"[^"]+"' \
      | sed -E 's/.*"(https?:[^"]+)".*/\1/' | grep -iE "$2" | head -n1
  fi
}

# Resolve the newest GitHub *release* asset whose filename matches a regex.
# Usage: _github_latest_asset "owner/repo" "regex"   ->  prints the URL (or nothing)
# Uses the GitHub REST API (JSON) — robust, structured, no page scraping. The
# unauthenticated API allows ~60 requests/hour, which is ample for this script.
# Strategy: try the release the maintainer flagged "latest" first; if it carries
# no matching asset (some projects don't tag the .deb-bearing release as latest,
# e.g. XDM), fall back to scanning recent releases (newest first) for a match.
_github_latest_asset() {
  local repo="$1" rx="$2" json url
  json="$(curl -fsSL --retry 3 -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/${repo}/releases/latest" 2>>"$LOG")" || json=""
  url="$(_gh_pick_asset "$json" "$rx")"
  if [ -z "$url" ]; then
    json="$(curl -fsSL --retry 3 -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/${repo}/releases?per_page=30" 2>>"$LOG")" || return 1
    url="$(_gh_pick_asset "$json" "$rx")"
  fi
  [ -n "$url" ] && printf '%s\n' "$url"
}

# Install (or refresh) a snap, bootstrapping snapd on demand. Returns non-zero if
# snap is unavailable (e.g. blocked by the distro) so the caller falls through.
_try_snap() {  # _try_snap NAME [--classic]
  local sname="$1" classic="${2:-}"
  if ! have_cmd snap; then
    apt_run "installing snapd" install snapd || return 1
    snap wait system seed.loaded 2>/dev/null || true
  fi
  have_cmd snap || return 1
  if snap list 2>/dev/null | grep -q "^${sname} "; then
    run_spin "refreshing snap ${sname}" snap refresh "$sname" || true
    return 0
  fi
  run_spin "installing snap ${sname}" snap install "$sname" $classic
}

# install_app "Display Name" [options...]
#   --apt "pkg1 pkg2"          APT package(s)
#   --snap NAME [--snap-classic]   snap name (optionally a classic snap)
#   --flatpak APPID            Flathub application id
#   --gh "owner/repo:regex"    newest matching GitHub release asset (.deb)
#   --deb URL                  direct vendor .deb URL
#   --manual "text"            manual-download prompt instructions (installs the .deb)
#   --webapp URL [--icon NAME] create a web-app .desktop shortcut as the last resort
#   --check "cmd"              shell test; if it succeeds the app is treated as present
#   --note "text"              curated "Windows ------> Linux alt" message for the log
install_app() {
  local name="$1"; shift
  local apt_list="" snap_name="" snap_classic="" flatpak_id="" gh_spec=""
  local deb_url="" manual_txt="" webapp_url="" icon="" check_cmd="" note=""
  while [ $# -gt 0 ]; do
    case "$1" in
      --apt)          apt_list="$2";     shift 2 ;;
      --snap)         snap_name="$2";    shift 2 ;;
      --snap-classic) snap_classic="--classic"; shift ;;
      --flatpak)      flatpak_id="$2";   shift 2 ;;
      --gh)           gh_spec="$2";      shift 2 ;;
      --deb)          deb_url="$2";      shift 2 ;;
      --manual)       manual_txt="$2";   shift 2 ;;
      --webapp)       webapp_url="$2";   shift 2 ;;
      --icon)         icon="$2";         shift 2 ;;
      --check)        check_cmd="$2";    shift 2 ;;
      --note)         note="$2";         shift 2 ;;
      *) warn "install_app: unknown option '$1' (ignored)"; shift ;;
    esac
  done

  log "$name"
  local shown="${note:-$name}"

  # Already installed?  (caller-supplied detector)
  if [ -n "$check_cmd" ] && eval "$check_cmd" >/dev/null 2>&1; then
    info "already installed: $shown"; mark_ok "$name" "already present"; return 0
  fi

  local tried=()

  # ---------- package managers, in order of significance ----------
  if [ -n "$apt_list" ]; then
    tried+=("apt")
    if apt_pkgs $apt_list; then info "installed: $shown via APT"; mark_ok "$name" "apt"; return 0; fi
  fi
  if [ -n "$snap_name" ]; then
    tried+=("snap")
    if _try_snap "$snap_name" "$snap_classic"; then info "installed: $shown via Snap"; mark_ok "$name" "snap"; return 0; fi
  fi
  if [ -n "$flatpak_id" ]; then
    tried+=("flatpak")
    if install_flatpak "$flatpak_id"; then info "installed: $shown via Flatpak"; mark_ok "$name" "flatpak"; return 0; fi
  fi

  # ---------- other methods ----------
  if [ -n "$gh_spec" ]; then
    tried+=("github-release")
    local repo="${gh_spec%%:*}" rx="${gh_spec#*:}" url
    if url="$(_github_latest_asset "$repo" "$rx")" && [ -n "$url" ]; then
      info "resolved latest release: $(basename "$url")"
      if install_deb_url "$name" "$url"; then info "installed: $shown via GitHub release .deb"; mark_ok "$name" "github .deb"; return 0; fi
    else
      warn "could not resolve a GitHub release asset for $name ($repo)"
    fi
  fi
  if [ -n "$deb_url" ]; then
    tried+=("deb-url")
    if install_deb_url "$name" "$deb_url"; then info "installed: $shown via vendor .deb"; mark_ok "$name" "vendor .deb"; return 0; fi
  fi
  if [ -n "$manual_txt" ]; then
    tried+=("manual")
    local f="$DOWNLOAD_DIR/${name// /_}.deb"
    if prompt_for_file "$name" "$f" "$manual_txt"; then
      if apt_run "installing $name" install "$f"; then info "installed: $shown via manual download"; mark_manual "$name"; return 0; fi
    fi
  fi
  if [ -n "$webapp_url" ]; then
    tried+=("webapp")
    if [ -n "${SUDO_USER:-}" ] && [ "$SUDO_USER" != "root" ] && command -v _webapp_desktop >/dev/null 2>&1; then
      USER_HOME="$(getent passwd "$SUDO_USER" | cut -d: -f6)"
      if _webapp_desktop "$name" "$webapp_url" "${icon:-web-browser}"; then
        mark_ok "$name" "web-app shortcut"; return 0
      fi
    else
      warn "web-app shortcut for $name needs a desktop user — skipped"
    fi
  fi

  mark_fail "$name" "all methods failed (tried: ${tried[*]:-none})"
  return 1
}
...
step "Core tools + build deps" apt_pkgs \
  apt-transport-https ca-certificates curl wget gnupg lsb-release unzip jq \
...
detect_pkg_mgrs; info "Package managers available (most-significant first): ${PKG_MGRS:-apt}"
...
NPP_VERSION="8.7.9"                         # fallback only — the live URL is resolved from the GitHub API
...
_notepadpp_wine() {
  ...
  # Resolve the newest Notepad++ installer .exe live from the GitHub API; fall
  # back to the pinned v${NPP_VERSION} URL only if the API lookup fails.
  local url ver
  url="$(_github_latest_asset notepad-plus-plus/notepad-plus-plus 'npp\.[0-9.]+\.Installer\.exe$')"
  if [ -z "$url" ]; then
    warn "Notepad++ GitHub API lookup failed — using pinned v${NPP_VERSION}"
    url="https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v${NPP_VERSION}/npp.${NPP_VERSION}.Installer.exe"
  fi
  ver="$(basename "$url" | grep -oE '[0-9]+\.[0-9]+(\.[0-9]+)?' | head -n1)"; [ -z "$ver" ] && ver="$NPP_VERSION"
  NPP_EXE="$DOWNLOAD_DIR/$(basename "$url")"
  if download_file "downloading Notepad++ installer (v${ver})" "$url" "$NPP_EXE"; then
    if [ -n "${SUDO_USER:-}" ] && [ "$SUDO_USER" != "root" ]; then
      local USER_HOME
      USER_HOME="$(getent passwd "$SUDO_USER" | cut -d: -f6)"
    ...
    info "installed: Notepad++ via Wine (v${ver})"
  else
    warn "download failed: Notepad++ v${ver}"
    return 1
  fi
}
...
# =============================================================================
# 3E. PEAZIP (additional rar-support native app alongside p7zip/unrar)
#     PeaZip is a FOSS archiver GUI that handles RAR, 7z, ZIP, and more.
#     Installed via the multi-method resolver: newest GitHub release .deb first,
#     Flatpak as the fallback. (Was a hard-coded peazip_11.4.0... filename before,
#     which broke whenever PeaZip published a newer release.)
# =============================================================================
install_app "PeaZip" \
  --check "deb_installed peazip" \
  --gh "peazip/PeaZip:peazip_.*LINUX\.GTK2.*amd64\.deb" \
  --flatpak "io.github.peazip.PeaZip" \
  --note "WinRAR ------> PeaZip (native RAR/ZIP/7z GUI) (alt, native)"
...
# XDM (Xtreme Download Manager): open-source (GitHub), newest release .deb resolved
# LIVE from the GitHub API (no hard-coded version). Complements uGet (APT) + IDM (Wine).
install_app "XDM (Xtreme Download Manager)" \
  --check "have_cmd xdman || deb_installed xdman" \
  --gh "subhra74/xdm:xdman_gtk_.*amd64\.deb"
