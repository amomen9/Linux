...
# Windows apps that were detected but NOT found in the manifest (no macOS mapping).
UNMATCHED_APPS=(
...
show_config() {  # show_config CONFIG_FILE
  ...
  while IFS='=' read -r k v; do
    ...
    case "$k" in
      ...
      UPDATE_EXISTING)   label="Update apps already installed on macOS?" ;;
...
# Warn about Windows apps with no manifest entry (no macOS equivalent was mapped).
show_unmatched_warning() {
  ...
  warn "so no macOS equivalent will be installed for them:"
  local a
  ...
  warn "then re-run run_project.ps1 on your Windows machine and use the regenerated 'Execute on macOS!' scripts."
...
install_manual_apps() {
  ...
  for entry in "${MANUAL_APPS[@]}"; do
    ...
    info "Then type its path in single quotes -- either a full path, or one relative to ${TARGET_HOME}/Downloads."
    info "Any valid macOS installer works (.dmg/.pkg/.app/.zip/.tar.gz/.sh/...);"
    ...
    while :; do
      ...
      case "$ans" in
        ...
        *)
          f="$(resolve_user_path "$ans")"
          if [ -z "$f" ] || [ ! -f "$f" ] || [ ! -r "$f" ]; then
            printf "\033[1;31m  No readable file at that path -- type the path in single quotes (e.g. '%s/Downloads/app.dmg'), or skip.\033[0m\n" "$TARGET_HOME" > /dev/tty
            continue
          fi
          LAST_ERR=""
...
print_module_summary() {  # print_module_summary "Module name"
  local f="${RESULTS:-/tmp/migrate_to_macos_results.tsv}" m="$1"
...
print_combined_summary() {
  local f="${RESULTS:-/tmp/migrate_to_macos_results.tsv}"
  printf '\n\n'
  log "==================== FINAL SUMMARY ===================="
...
main() {
  ...
  log "Migrate to macOS  --  Run All"
  info "macOS          : ${OS_VERSION} (${OS_CODENAME})"
  info "Package manager: ${PM}"
  info "Architecture   : ${ARCH} (${ARCH_LABEL})"
  info ""
  info "Order of operations (only the stages you enable below actually run):"
  info "  1. Device-driver installation"
  info "  2. Config settings migration (pre-phase)"
  info "  3. Application migration"
  info "  4. Manual application downloads"
  info "  5. Config settings migration (post-phase: shortcuts/favourites, ~/.ssh, Contacts)"
  info "  6. User & application data restore"
  info "  7. Docker component migration"
  ...
  if [ "$use_cfg" -eq 1 ]; then
    ...
    if [ "$do_apps" -eq 1 ]; then
      ...
      MIGRATE_VERSION_MODE="$(ask_ab "(4/10) Want to install same version of exact equivalents from Windows to macOS or the latest version? (Apps data restore might not be applicable with 'same')" "Same version" "Latest version")"
      export MIGRATE_VERSION_MODE
      if ask "(5/10) Update apps that are already installed on macOS?" n; then MIGRATE_UPDATE_EXISTING=yes; else MIGRATE_UPDATE_EXISTING=no; fi
      export MIGRATE_UPDATE_EXISTING
      if ask "(6/10) Install security-suite equivalents (antivirus, extra firewall, etc.)? macOS already ships Gatekeeper, XProtect and FileVault, so many users skip these." n; then MIGRATE_INSTALL_SECURITY=yes; else MIGRATE_INSTALL_SECURITY=no; fi
