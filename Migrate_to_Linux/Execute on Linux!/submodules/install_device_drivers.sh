...
#  Best-effort and family-aware (apt/dnf/pacman/zypper/apk/emerge/slackpkg).
# =============================================================================
# =============================================================================
...
install_firmware() {
  ...
  # openSUSE and Slackware ship the firmware blobs as "kernel-firmware".
  case "$FAMILY" in suse|slackware) pkg=kernel-firmware ;; esac
...
install_gpu() {
  ...
  if printf '%s' "$gpu" | grep -qi nvidia; then
    ...
    case "$PM" in
      ...
      emerge)   mark_manual "NVIDIA driver" "set VIDEO_CARDS=\"nvidia\" in /etc/portage/make.conf, then: emerge x11-drivers/nvidia-drivers" ;;
      slackpkg) mark_manual "NVIDIA driver" "install nvidia-driver + nvidia-kernel from SlackBuilds.org (sbopkg)" ;;
      apk)      mark_manual "NVIDIA driver" "Alpine (musl) has no official NVIDIA proprietary driver; use the open-source nouveau driver (in-kernel)" ;;
      *)        mark_manual "NVIDIA driver" "install the proprietary NVIDIA driver from your distribution's repositories" ;;
    esac
...
install_printing() {
  ...
  case "$PM" in
    ...
    emerge)   capture pm_install net-print/cups media-gfx/sane-backends || rc=1 ;;
    slackpkg) capture pm_install cups sane || rc=1 ;;
    apk)      capture pm_install cups sane || rc=1 ;;
  esac
  # Enable the CUPS daemon on systemd, OpenRC (Alpine / Gentoo) or SysV -- best-effort.
  if [ "$rc" -eq 0 ]; then
    systemctl enable --now cups 2>/dev/null \
      || { rc-update add cupsd 2>/dev/null; rc-service cupsd start 2>/dev/null; } || true
    mark_ok "printing/scanning (CUPS+SANE)"
